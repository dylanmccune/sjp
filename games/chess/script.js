console.clear();
const funFacts = [
  "Homeless people in California account for 28% of homeless people in the US",
  "Nearly 40% of homeless people in California are black or African American",
  "The San Francisco Bay Area has about 22% of California's homeless population",
  "Over 30% of former foster youth have experienced homelessness by 26",
  "The average price of a home in California is over twice the US average"
];
document.getElementById("fun-fact-close").addEventListener("click", () => {
  document.getElementById("fun-fact-modal").classList.remove("active");
});
function showFunFact() {
  const modal = document.getElementById("fun-fact-modal");
  const text = document.getElementById("fun-fact-text");
  text.textContent = funFacts[Math.floor(Math.random() * funFacts.length)];
